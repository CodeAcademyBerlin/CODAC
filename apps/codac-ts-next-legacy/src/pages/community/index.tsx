            ...
            {mentors.map((mentor: MentorEntity, i: Number) => {
              return mentor.attributes ? (
                <MentorsContent key={mentor.id}>
                  <Link className="noDeco" href={"/community/mentors/" + mentor.id} key={mentor.id}>
                    <Image
                      alt={mentor.attributes?.user?.data?.attributes?.firstname || "avatar"}
                ...
                >
                  <CohortContent>
                    <Image
                      width={50}
                      height={50}
                      src={cohort.attributes.logo?.data?.attributes?.url || logo}
                        ...
                        <StudentContent key={student.id}>
                          <div style={{ display: "flex" }}>
                            <Image
                              ...
                              height={50}
                              src={
                                student.attributes?.user?.data?.attributes?.avatar?.data?.attributes
                                  ?.url || logo
                              }
                              alt={
                                student.attributes?.user?.data?.attributes?.firstname +
                                  " " +
                                  student.attributes?.user?.data?.attributes?.lastname || "Student"
                              }
                            />
                            <Link
                              className="noDeco"
                              href={"/community/students/" + student?.attributes?.user?.data?.id}
