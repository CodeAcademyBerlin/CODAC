...
const List = ({ items, depth }) => {
    return (<div>
      {items.map((item, i) => {
            const isLast = i === items.length - 1;
            return (<div key={item.name} className={depth === 0
                    ? undefined // Ignore first level
                    : clsx("relative ml-5 pt-2", 
            ...
            <div className="flex gap-x-1">
              <div className={clsx("rounded-md px-2 py-0.5 text-xs tracking-wide", {
                    "bg-codac-blue text-blue-100": item.type === "client",
              ...
              <div className={clsx("rounded-md bg-gray-800 px-2 py-0.5 text-xs tracking-wide text-white/50", {
...
// server) in a tree
const sum = (items, componentType) => items.reduce((total, item) => 
// running total
total +
    ...
    (item.children ? sum(item.children, componentType) : 0), 0);
export const ComponentTree = ({ items }) => {
    ...
    return (<Boundary animateRerendering={false} labels={["Component Tree"]}>
      <div className="space-y-6">
          ...
          <div className="space-y-6">
            <div className="space-y-3 rounded-lg bg-gray-900 p-4">
              ...
              <div className="overflow-hidden rounded-full bg-gray-700">
                <div className={clsx("bg-codac-blue h-2 animate-[translateXReset_1s_ease-in-out_1_reverse] rounded-full")} style={{
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-x-3 text-sm text-gray-400">
                <div className="bg-codac-blue rounded-md px-2 py-0.5 text-xs tracking-widest text-white/50">
                  {"</>"}
                </div>
        ...
        <div className="text-codac-orange text-sm italic">
